...
import { Loader2, BookOpen, Newspaper, Search as SearchIcon, ChevronDown, ChevronUp } from "lucide-react";
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-center pb-1">
        <button
          ...
          className="border-2 border-black rounded-md px-3 py-1.5 inline-flex items-center justify-center hover:bg-gray-100 transition-colors"
          style={{ backgroundColor: "#ffffff" }}
          aria-expanded={!isCollapsed}
        >
          <img src={openAlexLogo} alt="OpenAlex" className="brand-asset h-7 w-auto object-contain" />
          {isCollapsed ? <ChevronDown className="h-4 w-4 ml-2 text-black" /> : <ChevronUp className="h-4 w-4 ml-2 text-black" />}
        </button>
      </div>
      {!isCollapsed && (
        <>
